...
__global__
void matrix_calc(double* A, double* Anew, size_t size)
{
	unsigned int j = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int i = blockIdx.y * blockDim.y + threadIdx.y;
	if (i * size + j > size * size) return;
	if((j == 0 || i == 0 || j >= size - 1 || i >= size - 1) == 0) Anew[i * size + j] = 0.25 * (A[i * size + j - 1] + A[(i - 1) * size + j] + A[(i + 1) * size + j] + A[i * size + j + 1]);		
}


__global__ 
void error_calc(double *Anew, double *A,double *error,int size)
{
    unsigned int j = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int i = blockIdx.y * blockDim.y + threadIdx.y;
    if (i * size + j > size * size) return;
...
int main(int argc, char const *argv[])
{
	...
	while (error > precision && iter < iter_max)
    {
		...
		else
        {
            ...
            {
				matrix_calc<<<grid, block, 0, stream>>>(Anew_device, A_device, n);
			...
			matrix_calc<<<grid, block, 0, stream>>>(Anew_device,A_device,n);
			error_calc<<<grid, block, 0, stream>>>(Anew_device,A_device,error_device,n);
